...
const savedColors = ['FBD668', 'FDBFBF', getRandomHexColor(), getRandomHexColor(), getRandomHexColor(), getRandomHexColor(), getRandomHexColor(), getRandomHexColor(), getRandomHexColor(), getRandomHexColor()];
const presets = {
  1: {
...
function updateOutputText(event) {
  ...
  if (format.outputPrefix) {
    ...
    if (nickName.value) {
      ...
      if (!nickName.value.match(letters)) nickName.value = 'https://self-fox.github.io/index/';
    }
  }

  let newNick = nickName.value
  if (!newNick) {
    newNick = 'Ты забыл Текст Написать'
  }
